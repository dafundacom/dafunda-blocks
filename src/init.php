...
function dbe_advanced_heading_add_assets()
{
	...
	wp_enqueue_style(
		"dafunda_blocks-advanced-heading-fonts",
	...
	if (!$output) {
		...
		foreach ((array) $widget_elements as $key => $widget_element) {
			if (!empty($widget_element["content"])) {
				$widget_blocks = dbe_getPresentBlocks(
		...
		if (!($main_assets_loaded && $advanced_heading_assets_loaded)) {
			...
			foreach ($presentBlocks as $block) {
				if (strpos($block["blockName"], "dbe/") === 0) {
					dbe_load_assets();
					if (
						strpos($block["blockName"], "dbe/advanced-heading") ===
						0
					) {
						dbe_advanced_heading_add_assets();
						break;
...
function dbe_include_block_attribute_css()
{
	...
	$presentBlocks = array_unique(
		array_merge(
			dbe_getPresentBlocks(),
			dbe_generate_widget_block_list(true)
		),
		SORT_REGULAR
	);
	$blockStylesheets = "";
		...
		if (
			...
			switch ($block["blockName"]) {
				...
				case "dbe/advanced-video":
					...
					if (
						array_key_exists("shadow", $attributes) &&
						...
						$blockStylesheets .= $prefix =
							...
							"px " .
							$attributes["shadow"][0]["blur"] .
							"px " .
							$attributes["shadow"][0]["spread"] .
							"px " .
							"rgba(" .
							hexdec(
								substr($attributes["shadow"][0]["color"], 1, 2)
							) .
							", " .
							hexdec(
								substr($attributes["shadow"][0]["color"], 3, 2)
							) .
							", " .
							hexdec(
								substr($attributes["shadow"][0]["color"], 5, 2)
							) .
							", " .
				...
				case "dbe/call-to-action-block":
					...
					$blockStylesheets .=
						...
						PHP_EOL .
						"color: " .
						($attributes["headColor"] ?: "inherit") .
						";" .
						PHP_EOL .
						"text-align: " .
						$attributes["headAlign"] .
						";" .
						PHP_EOL .
						"}" .
						PHP_EOL .
						...
						PHP_EOL .
						"color: " .
						($attributes["contentColor"] ?: "inherit") .
						";" .
						PHP_EOL .
						"text-align: " .
				...
				case "dbe/content-filter-block":
					...
					$blockStylesheets .=
						...
						PHP_EOL .
						"color: " .
						($attributes["activeButtonTextColor"] ?: "inherit") .
						";" .
						PHP_EOL .
						"}" .
						PHP_EOL;
				...
				case "dbe/content-toggle-block":
					...
					$blockStylesheets .=
						...
						PHP_EOL .
						"color: " .
						($attributes["titleLinkColor"] ?: "inherit") .
						";" .
						PHP_EOL .
						"}";
					break;
							...
							$blockStylesheets .=
								...
								implode(
									...
									array_fill(
										0,
										array_search(
											$attributes["smallestUnit"],
											$timeUnits
										) -
											array_search(
												$attributes["largestUnit"],
												$timeUnits
											) +
											1,
										"1fr"
				...
				case "dbe/how-to":
					...
					if ($attributes["useSections"]) {
						...
						$blockStylesheets .= implode(
							array_map(
								function ($section, $outerIndex, $prefix) {
									return implode(
										array_map(
											function (
												$stepPic,
				...
				case "dbe/progress-bar":
					...
					if ($attributes["barType"] === "linear") {
						...
						$strokeArcLength =
							($circlePathLength * $attributes["percentage"]) /
							100;
						$blockStylesheets .=
							$prefix .
				...
				case "dbe/review":
					...
					$blockStylesheets .=
						...
						PHP_EOL .
						"border-color: " .
						$attributes["callToActionBorderColor"] .
						";" .
						PHP_EOL .
						"background-color: " .
						$attributes["callToActionBackColor"] .
						";" .
						PHP_EOL .
				...
				case "dbe/social-share":
					...
					if (
						$attributes["useCaptions"] &&
						!$attributes["addOutline"]
					) {
						$blockStylesheets .=
							$prefix .
							" .social-share-icons.no-outline > div{" .
							PHP_EOL .
							"border: none;" .
							PHP_EOL .
							"}";
				...
				case "dbe/star-rating-block":
					...
					$blockStylesheets .=
						...
						PHP_EOL .
						"color: " .
						($attributes["reviewTextColor"] ?: "inherit") .
						";" .
						PHP_EOL .
						"}" .
						PHP_EOL .
				...
				case "dbe/styled-box":
					...
					if ($attributes["mode"] === "notification") {
						...
						if ($attributes["outlineRadiusUnit"] === "percent") {
							...
							$attributes["outlineRadiusUnit"] === "pixel"
						) {
							$radiusUnit = "px";
						...
						$blockStylesheets .=
							...
							PHP_EOL .
							"border-radius: " .
							$attributes["outlineRoundingRadius"] .
							$radiusUnit .
							";" .
							PHP_EOL .
							"background-color: " .
							($attributes["boxColor"] ?: "inherit") .
							";" .
							PHP_EOL .
							"}" .
							PHP_EOL;
				...
				case "dbe/styled-list":
					...
					$blockStylesheets .=
						...
						PHP_EOL .
						"top: " .
						($attributes["iconSize"] >= 5
							? 3
							: ($attributes["iconSize"] < 3
								? 2
								: 0)) .
						'px;
                            font-size: 1em;
                            height: ' .
						(4 + $attributes["iconSize"]) / 10 .
						'em; 
                            width: ' .
						(4 + $attributes["iconSize"]) / 10 .
						'em;
					...
					if ($attributes["columns"] > 1) {
						$blockStylesheets .=
							...
							PHP_EOL .
							"grid-template-columns: " .
							str_repeat("auto ", $attributes["columns"] - 1) .
							"auto;" .
							PHP_EOL .
							"}";
					}
					if (
						$attributes["columns"] > $attributes["maxMobileColumns"]
					) {
						$blockStylesheets .=
				...
				case "dbe/tabbed-content-block":
					...
					$blockStylesheets .=
						...
						PHP_EOL .
						"color: " .
						($attributes["normalTitleColor"] ?: "inherit") .
						";" .
						PHP_EOL .
						"}" .
						PHP_EOL .
...
function dafunda_blocks_cgb_editor_assets()
{
	...
	wp_enqueue_script(
		...
		["wp-blocks", "wp-element", "wp-components", "wp-editor", "wp-api"], // Dependencies, defined above.
		Dafunda_Blocks_Constants::plugin_version(),
	...
	if (
		...
		foreach ($blockList as $key => $block) {
			...
			if (
				...
				fwrite(
					$adminStyleFile,
					file_get_contents($blockDir . "feature-box" . "/editor.css")
				);
				fwrite(
					$adminStyleFile,
					file_get_contents($blockDir . "number-box" . "/editor.css")
				);
			}
		}
...
{
	register_setting("settings", "icon_choices", [
		"type" => "string",
		"show_in_rest" => true,
