...
function dbe_render_tab_block($attributes, $contents){
    ...
    return '<div role="tabpanel" class="wp-block-tabbed-content-tab-content-wrap '.
        ($isActive ? 'active' : 'hide') . (isset($className) ? ' ' . esc_attr($className) : '') . '"
        id="tabbed-content-' . $parentID . '-panel-' . $index . '" aria-labelledby="tabbed-content-' . $parentID . '-tab-' . $index . '" tabindex="0">'
        . $contents . '</div>';
}
...
function dbe_render_tabbed_content_block($attributes, $contents){
    ...
    foreach ($contents as $key => $content) {
        ...
        if(preg_match('/^<div role="tabpanel" class="wp-block-tabbed-content-tab-content-wrap active"/', $tabContent)){
    ...
    foreach($tabsTitle as $key=>$title){
        $tabs .= '<div role="tab" id="tabbed-content-' . $blockID . '-tab-' . $key . '" aria-controls="tabbed-content-' . $blockID . '-panel-' . $key . '"
            ...
            . '"' .($blockID === '' ? '' : ' id="tabbed-content-' . $blockID . '"')
             . ($mobileTabDisplay === 'accordion' || $tabletTabDisplay === 'accordion' ? ' data-active-tabs="[' . $activeTab . ']"' : '') . '>
                ...
                . ($tabletTabDisplay !== 'accordion' ? ' ' . $tabletTabStyle . '-tab-width-tablet' : '') . '">
                    ...
                    . ($mobileTabDisplay === 'accordion' ? ' mobile-hide' : ' ' . $blockName . '-tabs-title-mobile-' . $mobileTabStyle . '-tab' ) 
                    . ($tabletTabDisplay === 'accordion' ? ' tablet-hide' : ' ' . $blockName . '-tabs-title-tablet-' . $tabletTabStyle . '-tab') . '">' .
                    $tabs . '</div></div>
                <div class="' . $blockName . '-tabs-content' . ($tabVertical ? ' vertical-content-width ' : '')
                . ($mobileTabDisplay === 'verticaltab' ? ' vertical-content-width-mobile' : ($mobileTabDisplay === 'accordion' ? ' tabbed-content-mobile-accordion' : ''))
                . ($tabletTabDisplay === 'verticaltab' ? ' vertical-content-width-tablet' : ($tabletTabDisplay === 'accordion' ? ' tabbed-content-tablet-accordion' : ''))
                . '">' .
