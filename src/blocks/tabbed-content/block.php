...
function dbe_render_tab_block($attributes, $contents){
    extract($attributes);
    return '<div role="tabpanel" class="wp-block-dbe-tabbed-content-tab-content-wrap '.
        ($isActive ? 'active' : 'dbe-hide') . (isset($className) ? ' ' . esc_attr($className) : '') . '"
        id="dbe-tabbed-content-' . $parentID . '-panel-' . $index . '" aria-labelledby="dbe-tabbed-content-' . $parentID . '-tab-' . $index . '" tabindex="0">'
        . $contents . '</div>';
}
...
function dbe_register_tab_block(){
    if(function_exists('register_block_type')){
        ...
        register_block_type('dbe/tab-block', array(
...
function dbe_render_tabbed_content_block($attributes, $contents){
    extract($attributes);
    $blockName = 'wp-block-dbe-tabbed-content';
    ...
    $contents = str_get_html('<div id="tabarray">' . $contents . '</div>', $lowercase=true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=false)
                    ->find('#tabarray > .wp-block-dbe-tabbed-content-tab-content-wrap');
    ...
    foreach ($contents as $key => $content) {
        ...
        if(preg_match('/^<div role="tabpanel" class="wp-block-dbe-tabbed-content-tab-content-wrap active"/', $tabContent)){
        ...
        if($tabletTabDisplay === 'accordion' || $mobileTabDisplay === 'accordion'){
            ...
            ($tabletTabDisplay === 'accordion' ? ' dbe-tablet-display' : '') .
            ($mobileTabDisplay === 'accordion' ? ' dbe-mobile-display' : '') .
    ...
    foreach($tabsTitle as $key=>$title){
        $tabs .= '<div role="tab" id="dbe-tabbed-content-' . $blockID . '-tab-' . $key . '" aria-controls="dbe-tabbed-content-' . $blockID . '-panel-' . $key . '"
    ...
    return '<div class="' . $blockName . ($tabStyle !== 'tabs' ? '-' . $tabStyle : '') . ' ' . $blockName . '-holder' . ($tabVertical ? ' vertical-holder' : '')
            ...
            . '"' .($blockID === '' ? '' : ' id="dbe-tabbed-content-' . $blockID . '"')
             . ($mobileTabDisplay === 'accordion' || $tabletTabDisplay === 'accordion' ? ' data-active-tabs="[' . $activeTab . ']"' : '') . '>
                ...
                . ($tabletTabDisplay !== 'accordion' ? ' ' . $tabletTabStyle . '-tab-width-tablet' : '') . '">
                    ...
                    . ($mobileTabDisplay === 'accordion' ? ' dbe-mobile-hide' : ' ' . $blockName . '-tabs-title-mobile-' . $mobileTabStyle . '-tab' ) 
                    . ($tabletTabDisplay === 'accordion' ? ' dbe-tablet-hide' : ' ' . $blockName . '-tabs-title-tablet-' . $tabletTabStyle . '-tab') . '">' .
                    $tabs . '</div></div>
                <div class="' . $blockName . '-tabs-content' . ($tabVertical ? ' vertical-content-width ' : '')
                . ($mobileTabDisplay === 'verticaltab' ? ' vertical-content-width-mobile' : ($mobileTabDisplay === 'accordion' ? ' dbe-tabbed-content-mobile-accordion' : ''))
                . ($tabletTabDisplay === 'verticaltab' ? ' vertical-content-width-tablet' : ($tabletTabDisplay === 'accordion' ? ' dbe-tabbed-content-tablet-accordion' : ''))
                . '">' .
...
function dbe_register_tabbed_content_block(){
    if(function_exists('register_block_type')){
        require dirname(dirname(__DIR__)) . '/defaults.php';
        register_block_type('dbe/tabbed-content-block', array(
            'attributes' => $defaultValues['dbe/tabbed-content-block']['attributes'],
...
function dbe_tabbed_content_add_frontend_assets() {
    ...
    foreach( $presentBlocks as $block ){
        if($block['blockName'] === 'dbe/tabbed-content' || $block['blockName'] === 'dbe/tabbed-content-block'){
            wp_enqueue_script(
