...
function dbe_getNodeindex(elem) {
...
function dbe_handleTabEvent(tab) {
	...
	var activeStyle = parent
		.querySelector(
			".wp-block-dbe-tabbed-content-tab-title-".concat(
				isVertical ? "vertical-" : "",
				"wrap.active"
			)
		)
...
function dbe_checkPrevTab(event) {
	...
	if (event.target.previousElementSibling) {
		...
		dbe_focusOnLastTab(event);
	}
}

function dbe_checkNextTab(event) {
	event.preventDefault();
	...
	if (event.target.nextElementSibling) {
		...
		dbe_focusOnFirstTab(event);
	}
}

function dbe_focusOnFirstTab(event) {
	event.preventDefault();
	event.target.setAttribute("tabindex", -1);
	event.target.parentElement.children[0].focus();
}

function dbe_focusOnLastTab(event) {
	event.preventDefault();
	event.target.setAttribute("tabindex", -1);
...
function dbe_commonKeyPress(event) {
	if (event.key === "Home") {
		dbe_focusOnFirstTab(event);
...
function dbe_upDownPress(event) {
	if (event.key === "ArrowUp") {
		dbe_checkPrevTab(event);
	} else if (event.key === "ArrowDown") {
		dbe_checkNextTab(event);
	} else {
		dbe_commonKeyPress(event);
	}
}

function dbe_leftRightPress(event) {
...
function dbe_switchFocusToTab(index, tabBar) {
						...
						switch (displayModes[i][transitionFrom - 1]) {
							...
							case "verticaltab":
								Array.prototype.slice
									...
									.forEach(function (tab) {
										tab.removeEventListener("keydown", dbe_upDownPress);
									});
								break;
							...
							default:
								Array.prototype.slice
									.call(tabBar.children)
									.forEach(function (tab) {
										tab.removeEventListener("keydown", dbe_leftRightPress);
									});
								break;
						}
						...
						switch (displayModes[i][transitionTo - 1]) {
							...
							case "verticaltab":
								Array.prototype.slice
									.call(tabBar.children)
									.forEach(function (tab) {
										tab.addEventListener("keydown", dbe_upDownPress);
									});
							...
							default:
								...
								dbe_switchFocusToTab(newActiveTab, tabBar);
								Array.prototype.slice
									.call(tabBar.children)
									.forEach(function (tab) {
										tab.addEventListener("keydown", dbe_leftRightPress);
									});
	...
	if (targetElement) {
		if (
			...
			var displayModes = ancestorTabBlocks.map(function (a) {
				return dbe_getTabbedContentDisplayModes(a);
			});
			var displayMode = -1;
			...
			ancestorTabBlocks.forEach(function (a, i) {
				...
				if (displayModes[i][displayMode] === "accordion") {
					...
					if (
						...
						dbe_switchFocusToTab(ancestorTabIndexes[i], tabBar);
					}
				}
			});
...
function dbe_initializeTabBlock(instance) {
	var tabBar = instance.children[0].children[0];
	...
	var displayModes = dbe_getTabbedContentDisplayModes(instance);
