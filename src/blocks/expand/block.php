...
{
	extract($attributes);
	return '<div class="expand-portion expand-' .
		$displayType .
...
function dbe_register_expand_portion_block($attributes)
{
	if (function_exists("register_block_type")) {
		...
		register_block_type("dbe/expand-portion", [
...
function dbe_expand_block_add_frontend_assets()
{
	...
	foreach ($presentBlocks as $block) {
		if (
			$block["blockName"] === "dbe/expand" ||
			$block["blockName"] === "dbe/expand-portion"
		) {
			wp_enqueue_script(
