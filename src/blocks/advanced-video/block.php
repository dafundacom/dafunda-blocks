...
function dbe_render_advanced_video_block($attributes)
{
	...
	return '<div id="advanced-video-' .
		$blockID .
		'" class="advanced-video-container">' .
		(!in_array($videoSource, ["local", "unknown", "videopress"]) &&
		$thumbnail !== ""
			? '<img class="advanced-video-thumbnail" height="' .
				$height .
				'" width="' .
				$width .
				'" src="' .
...
function dbe_register_advanced_video_block()
{
	if (function_exists("register_block_type")) {
		...
		register_block_type("dbe/advanced-video", [
