...
export const inspectorControls = (props) => {
	const BUTTON_SIZES = {
		small: __("S", "dafunda-blocks"),
		medium: __("M", "dafunda-blocks"),
		large: __("L", "dafunda-blocks"),
		larger: __("XL", "dafunda-blocks"),
	};

	const BUTTON_WIDTHS = {
		fixed: __("Fixed", "dafunda-blocks"),
		flex: __("Flexible", "dafunda-blocks"),
		full: __("Full", "dafunda-blocks"),
	};
				...
				<PanelBody title={__("Button Style", "dafunda-blocks")}>
					...
					{buttons[activeButtonIndex].buttonRounded && (
						<div id="button-radius-panel">
							...
							/>
							<ButtonGroup
								aria-label={__("Button Radius Unit", "dafunda-blocks")}
							>
								{["px", "%"].map((b) => (
									<Button
										isLarge
					...
					<ToggleControl
						label={__("Transparent", "dafunda-blocks")}
				...
				<PanelBody title={__("Button Icon", "dafunda-blocks")}>
					<div className="button-grid">
							...
							<Dropdown
								...
								renderToggle={({ isOpen, onToggle }) => (
									<Button
										...
										label={__("Open icon selection dialog", "dafunda-blocks")}
										onClick={onToggle}
						...
						<SelectControl
							...
							options={[
								{
									label: __("Left", "dafunda-blocks"),
									value: "left",
								},
								{
									label: __("Right", "dafunda-blocks"),
									value: "right",
								},
							]}
							onChange={(pos) =>
...
export class EditorComponent extends Component {
	...
	loadIconList() {
		...
		loadPromise.then(() => {
			...
			this.settings.fetch().then((response) => {
				...
				if (response.dbe_icon_choices !== "") {
					const currentTime = ~~(Date.now() / 1000);

					//trim old entries from frequenticons that are older than two weeks
					frequentIcons = JSON.parse(response.dbe_icon_choices)
						.map((f) => ({
							name: f.name,
				...
				if (frequentIcons.length) {
					...
					//check if anything from dbe_icon_choices has been trimmed in frequentIcons
					if (JSON.stringify(frequentIcons) !== response.dbe_icon_choices) {
						const newIconArray = new models.Settings({
							dbe_icon_choices: JSON.stringify(frequentIcons),
						});
						newIconArray.save();
					}
	...
	updateIconList() {
		...
		const newIconArray = new models.Settings({
			dbe_icon_choices: JSON.stringify(iconPrefs),
		});
		newIconArray.save();
	}
