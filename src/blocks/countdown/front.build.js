			...
			timer[i] = setInterval(function () {
				...
				if (!initialValue[i]) {
					...
					if (largestUnit && smallestUnit) {
						var amounts = Array(timeUnits.indexOf(largestUnit))
							.fill(0)
							.concat(
								initialValue[i].map(function (arr) {
						...
						if (
							timeLeft >
							amounts.reduce(function (total, current, j) {
								return (
									total +
									current *
										conversionFactor
											.slice(j, 4)
											.reduce(function (curFactor, current) {
				...
				if (timeLeft >= 0) {
					if (instance.querySelector(".dbe-countdown-odometer-container")) {
						...
						var maxValues = [0, 6, 23, 59, 59].slice(
							_timeUnits.indexOf(largestUnit),
							_timeUnits.indexOf(smallestUnit) + 1
						);
						...
						var replacements = [weeks, days, hours, minutes, seconds]
							.slice(
								_timeUnits.indexOf(largestUnit),
								_timeUnits.indexOf(smallestUnit) + 1
							)
							.map(function (r, j) {
								return breakIntoDigits(
									r,
									Math.floor(
										Math.log10(
											maxValues[j] ||
						...
						initialValue[i].forEach(function (display, j) {
							if (
								display.every(function (digit, k) {
								...
								var addExtraZeroes = function addExtraZeroes(
									arr,
									targetLength
								) {
									return [].concat(
								...
								if (animationDirection === "increase") {
									...
									var prevDigits = [];
									incomingDigits.push(
										replacements[j].map(function (d, k) {
											var currentMax =
												display[k - 1] === maxDigits[k - 1] ? maxDigits[i] : 9;

											if (prevDigits.length > 1) {
												var prevDigits2 = prevDigits.slice(
													1,
													prevDigits.length - 1
												);
									...
									var _maxDigits = breakIntoDigits(
									...
									incomingDigits.push(
										replacements[j].map(function (d, k) {
											var currentMax =
												replacements[j][k - 1] === _maxDigits[k - 1]
													? _maxDigits[k]
													: 9;

											if (_prevDigits.length > 1) {
												var prevDigits2 = _prevDigits.slice(
													1,
													_prevDigits.length - 1
												);

												var cycle = prevDigits2.map(function (p) {
													return integerArray(
														0,
														_maxDigits[k - 1] === p ? _maxDigits[k] : 9
													);
												});
											...
											if (d === display[k]) {
												if (newValue < currentValue) {
													...
													_prevDigits = integerArray(d, currentMax).concat(
														_extraDigits,
														integerArray(0, d)
													);
												}
						...
						var odometerSlot = Array.prototype.slice
							.call(instance.querySelectorAll(".dbe-countdown-odometer"))
							.map(function (a) {
								return Array.prototype.slice.call(a.children);
							});
						var finishedTransitions = 0;
						var transitionCount = incomingDigits
							.reduce(function (collection, currentArray) {
							...
							maxValues.forEach(function (m, j) {
								if (m === 0) {
									...
									if (initialValue[i][j][0] === 0) {
										...
										var targetLength =
											(curVal ? Math.floor(Math.log10(curVal)) : 0) + 1; //eliminate element containing extra zero
							...
							arr.forEach(function (d, k) {
								if (Array.isArray(d)) {
									...
									odometerSlot[j][k].innerHTML = d
										.map(function (dd) {
											return '<div class="odometer-moving-digit">'.concat(
												dd,
												"</div>"
											);
										})
									...
									if (animationDirection === "decrease") {
										odometerSlot[j][k].style.transform = "translateY(".concat(
											100 * (1 / d.length - 1),
											"%)"
										);
									} //animate
