...
function dbe_render_rangked_list_block($attributes)
{
	extract($attributes);
	ob_start();
	?>
	<div class="rangked-list" id="rangked-list-<?= $blockID ?>">
		<ol class="rangked-list p-0">
			<?php foreach ($lists as $index => $list): ?>
				<li class="rangked-list-card flex flex-wrap p-0 relative mb-6">
	...
	"imagealt"
       	...
       	"title"
						...
						<!-- Vote Form -->
						<div class="rangked-list-vote ml-3">
					...
					<!-- <button class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 h-fit" type="button" data-modal-target="rangked-list-modal-<?= $index ?>">
						Toggle modal
					</button> -->
					...
					<svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 mt-1 text-gray-500 hover:text-gray-700 cursor-pointer" viewBox="0 0 20 20" fill="currentColor">
						<path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"  data-modal-target="rangked-list-modal-<?= $index ?>" />
					</svg>
				</li>
				<div class="relative modal" aria-labelledby="modal-title" role="dialog" aria-modal="true" id="rangked-list-modal-<?= $index ?>">
	...
	<script>
		document.querySelectorAll(".rangked-list-vote .button").forEach((el, index) => {
			el.addEventListener("click", (e) => {
				e.preventDefault()
				e.stopPropagation()
				let parent = e.target.closest(".rangked-list-vote")
...
function dbe_register_rangked_list_block()
{
	if (function_exists("register_block_type")) {
		require dirname(dirname(__DIR__)) . "/defaults.php";
		register_block_type("dbe/rangked-list", [
			"attributes" => $defaultValues["dbe/rangked-list"]["attributes"],
			"render_callback" => "dbe_render_rangked_list_block",
		]);
	}
}
add_action("init", "dbe_register_rangked_list_block");
