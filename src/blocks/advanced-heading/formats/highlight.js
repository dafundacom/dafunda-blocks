...
/* format name */
let name = "dafunda-blocks/highlight";
    ...
    if (highlightedElements) {
      ...
      if (hasHightlight) {
        ...
        if (!attributesList["letter-spacing"]) {
          let parent = window.getComputedStyle(
            selectedBlock.firstElementChild
          ).letterSpacing;
  ...
  return (
    <>
      ...
      {highlightedElements && (
        <InspectorControls>
          <PanelBody
            title={__("Highlight Settings", "dafunda-blocks")}
            intialOpen={false}
          >
            {/* Background & Text Color */}
            <PanelColorSettings
              title={__("Highlight Colors", "dafunda-blocks")}
              colorSettings={[
                {
                  value: attributesList["color"],
                  onChange: (newColor) => onChangeAttribute(newColor, "color"),
                  label: __("Highlight Text Color", "dafunda-blocks"),
                },
                {
                  value: attributesList["background-color"],
                  onChange: (newColor) =>
                    onChangeAttribute(newColor, "background-color"),
                  label: __("Highlight Background Color", "dafunda-blocks"),
                },
              ]}
            />
            {/* Font Size */}
            <RangeControl
              label={__("Highlight Font Size", "dafunda-blocks")}
              value={parseInt(
            ...
            {/* Text Transform */}
            <SelectControl
              label={__("Highlight Text transform", "dafunda-blocks")}
              options={textTransformOptions}
              value={
                attributesList["text-transform"]
                  ? attributesList["text-transform"]
                  : __("None", "dafunda-blocks")
              }
            ...
            {/* Letter Spacing */}
            <RangeControl
              label={__("Highlight Letter Spacing", "dafunda-blocks")}
              value={parseInt(
            ...
            {/* Font Weight */}
            <SelectControl
              label={__("Highlight Font Weight", "dafunda-blocks")}
...
const exclusiveHighlight = compose(
  ...
  ifCondition(
    (props) =>
      props.selectedBlock && props.selectedBlock.name === "dbe/advanced-heading"
  )
)(highlightEdit);
