...
export class Edit_c extends Component {
  constructor(props) {
    super(props);
    this.state = {
      average:
  ...
  render() {
    ...
    return (
      <div className="review_block wp-block">
        <RichText
...
export function Edit_f(props) {
  ...
  //     average: this.props.items.map((i) => i.value).reduce((total, v) => total + v) / this.props.items.length,mouseOnHold: false
  // })
  ...
  let ctaButton = createRef();

  const newAverage =
    items.map((i) => i.value).reduce((total, v) => total + v) / items.length;

  if (average !== newAverage) {
    setAverage({ average: newAverage });
  }
  ...
  return () => {
    <div className="review_block wp-block" {...useBlockProps()}>
      <h1>REVIEW Fun</h1>
      <RichText
        className="review_item_name"
        placeholder={__("Title of the review")}
