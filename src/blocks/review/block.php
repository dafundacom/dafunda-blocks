...
function dbe_render_review_block($attributes)
{
	...
	$offerCode =
		...
		'",
        "priceCurrency": "' .
		dbe_filterJsonldString($offerCurrency) .
		'",' .
	...
	switch ($itemType) {
		case "Book":
			$itemExtras =
				'"author": "' .
				dbe_filterJsonldString($bookAuthorName) .
				'",
                            "isbn": "' .
				dbe_filterJsonldString($isbn) .
				'",
                            "sameAs": "' .
				esc_url($itemPage) .
				'"';
		...
		case "Course":
			$itemExtras =
				'"provider": "' . dbe_filterJsonldString($provider) . '"';
			break;
		case "Event":
			$itemExtras =
				$offerCode .
            ...
            "startDate": "' .
				...
				($usePhysicalAddress
					? '"Place",
                "name": "' .
						dbe_filterJsonldString($addressName) .
						'",
                "address": "' .
						dbe_filterJsonldString($address) .
						'"'
					: '"VirtualLocation",
                "url": "' .
						esc_url($eventPage) .
				...
				'},
            "organizer": "' .
				dbe_filterJsonldString($organizer) .
				'",
            "performer": "' .
				dbe_filterJsonldString($performer) .
				'"';
			break;
		case "Product":
			$itemExtras =
				'"brand": {
                                "@type": "Brand",
                                "name": "' .
				dbe_filterJsonldString($brand) .
				'"
                            },
                            "sku": "' .
				dbe_filterJsonldString($sku) .
				'",
                            "' .
				dbe_filterJsonldString($identifierType) .
				'": "' .
				dbe_filterJsonldString($identifier) .
				'",' .
				$offerCode;
			break;
		case "LocalBusiness":
			$itemExtras = isset($cuisines)
				...
				: "" .
					...
					'",
                            "telephone": "' .
					dbe_filterJsonldString($telephone) .
					'",
                            "priceRange": "' .
					dbe_filterJsonldString($priceRange) .
					'",
                            "sameAs": "' .
					esc_url($itemPage) .
					'"';
		...
		case "SoftwareApplication":
			$itemExtras =
				'"applicationCategory": "' .
				dbe_filterJsonldString($appCategory) .
				'",
                            "operatingSystem": "' .
				dbe_filterJsonldString($operatingSystem) .
				'",' .
				$offerCode;
			break;
		case "MediaObject":
	...
	return '<div class="review_block' .
        ...
        "@type": "Review",' .
					($useSummary
						? '"reviewBody": "' .
							dbe_filterJsonldString($summaryDescription) .
							'",'
						: "") .
					'"description": "' .
					dbe_filterJsonldString($description) .
					'",
        "itemReviewed": {
            "@type":"' .
					...
					'",' .
					($itemName
						? '"name":"' . dbe_filterJsonldString($itemName) . '",'
						: "") .
					($imgURL
						? ($itemSubtype === "VideoObject"
								? '"thumbnailUrl'
        ...
        "author":{
            "@type": "Person",
            "name": "' .
					dbe_filterJsonldString($authorName) .
					'"
        },
        "publisher": "' .
					dbe_filterJsonldString($reviewPublisher) .
					'",
        "datePublished": "' .
