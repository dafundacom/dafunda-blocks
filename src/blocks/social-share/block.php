...
function dbe_render_social_share_block($attributes)
{
	...
	$iconDetails = [
		"facebook" => dbe_get_facebook_icon(
			$attributes,
			$icon_size,
			$iconShape,
			$useCaptions ? $facebookCaption : "",
			$addOutline && $useCaptions
		),
		"twitter" => dbe_get_twitter_icon(
			$attributes,
			$icon_size,
			$iconShape,
			$useCaptions ? $twitterCaption : "",
			$addOutline && $useCaptions
		),
		"linkedin" => dbe_get_linkedin_icon(
			$attributes,
			$icon_size,
			$iconShape,
			$useCaptions ? $linkedInCaption : "",
			$addOutline && $useCaptions
		),
		"pinterest" => dbe_get_pinterest_icon(
			$attributes,
			$icon_size,
			$iconShape,
			$useCaptions ? $pinterestCaption : "",
			$addOutline && $useCaptions
		),
		"reddit" => dbe_get_reddit_icon(
			$attributes,
			$icon_size,
			$iconShape,
			$useCaptions ? $redditCaption : "",
			$addOutline && $useCaptions
		),
		"tumblr" => dbe_get_tumblr_icon(
			$attributes,
			$icon_size,
			$iconShape,
			$useCaptions ? $tumblrCaption : "",
			$addOutline && $useCaptions
		),
	];
	...
	return '<div class="wp-block-social-share' .
		(isset($className) ? " " . esc_attr($className) : "") .
		'"' .
		($blockID === "" ? "" : ' id="social-share-' . $blockID . '"') .
		'>
		<div class="social-share-icons align-icons-' .
		$align .
...
function dbe_prepare_social_share_icon(
	$icon,
	$iconShape,
	$siteName,
	$link,
	...
	if ($hasOutline) {
		return '<a target="_blank" href="' .
			...
			$siteName .
			'-container">
		<span class="social-share-icon social-share-' .
			$siteName .
			" " .
			$iconShape .
			'">' .
		...
		return ($caption
			? '<div class="social-share-' . $siteName . '-container">'
			: "") .
			'<a target="_blank" href="' .
			esc_url($link) .
			'" class="social-share-icon social-share-' .
			$siteName .
			" " .
			($caption ? " " : "social-share-standalone-icon ") .
			$iconShape .
			'">' .
			$icon .
			"</a>" .
...
function dbe_get_facebook_icon(
	$attributes,
	$icon_size,
	$iconShape,
	$caption,
	...
	return dbe_prepare_social_share_icon(
		$facebook_icon,
		$iconShape,
		"facebook",
		$facebook_url,
 ...
 * @return string
 */
function dbe_get_linkedin_icon(
	$attributes,
	$icon_size,
	$iconShape,
	$caption,
 ...
 * @return string
 */
function dbe_get_pinterest_icon(
	$attributes,
	$icon_size,
	$iconShape,
	$caption,
	...
	if (has_post_thumbnail()) {
		...
		$thumbnail = $thumbnail_id
			? current(wp_get_attachment_image_src($thumbnail_id, "large", true))
			: "";
	} else {
		$thumbnail = null;
	}
...
function dbe_register_social_share_block()
{
	if (function_exists("register_block_type")) {
		...
		register_block_type("dbe/social-share", [
