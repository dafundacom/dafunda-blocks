...
export class OldPanelContent extends Component {
	...
	render() {
		...
		if (newBlockTarget.length > 0) {
			...
			insertBlock(
				createBlock("dbe/content-toggle-panel", {
					theme: theme,
					collapsed: collapsed,
		...
		if (newArrangement !== oldArrangement) {
			...
			const childBlocks = this.getPanels()
				.filter((block) => block.name === "dbe/content-toggle-panel")
		...
		return [
			...
			<div className={className}>
				...
				<InnerBlocks
					template={[["dbe/content-toggle-panel"]]} //initial content
					templateLock={false}
					allowedBlocks={["dbe/content-toggle-panel"]}
				/>
			</div>,
		];
	}
...
export class PanelContent extends Component {
	...
	render() {
		...
		if (newBlockTarget.length > 0) {
			const { index, newBlockPosition } = newBlockTarget[0].attributes;
			insertBlock(
				createBlock("dbe/content-toggle-panel-block", {
		...
		if (newArrangement.length === 0) {
			...
			const childBlocks = this.props.block.innerBlocks
				.filter((block) => block.name === "dbe/content-toggle-panel-block")
				.map((panels) => panels.clientId);
		...
		return [
			...
			<div className={className} id={`dbe-content-toggle-${blockID}`}>
				<InnerBlocks
					templateLock={false}
					allowedBlocks={["dbe/content-toggle-panel-block"]}
				/>
				<style
