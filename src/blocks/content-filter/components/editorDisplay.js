...
export class OldPanelContent extends Component {
	...
	deleteFilterArrayItem(pos) {
		const { block, attributes, setAttributes, updateBlockAttributes } =
			this.props;
		const { filterArray } = attributes;
	...
	render() {
		...
		const newChildBlock = createBlock("dbe/content-filter-entry", {
		...
		return [
			...
			<div className="content-filter-main">
						...
						replaceBlock(
							...
							createBlock(
								...
								block.innerBlocks.map((innerBlock) =>
									createBlock(
										"dbe/content-filter-entry-block",
										{
											availableFilters: filterArray,
				...
				<InnerBlocks
					templateLock={false}
					allowedBlocks={["dbe/content-filter-entry"]}
				/>
				{filterArray.length > 0 &&
...
export class PanelContent extends Component {
	...
	render() {
		...
		const newChildBlock = createBlock("dbe/content-filter-entry-block", {
			availableFilters: filterArray,
			selectedFilters: filterArray.map((category) =>
				category.canUseMultiple
					? Array(category.filters.length).fill(false)
		...
		return [
			...
			<div className="content-filter-main">
				...
				<InnerBlocks
					templateLock={false}
					allowedBlocks={["dbe/content-filter-entry-block"]}
				/>
				{filterArray.length > 0 &&
					filterArray.filter((f) => f.filters.length > 0).length > 0 && (
						<button
