...
function dbe_render_content_filter_entry_block($attributes, $content)
{
	...
	return '<div class="content-filter-panel' .
		(isset($className) ? " " . esc_attr($className) : "") .
		($initiallyShow ? "" : " hide") .
		'" data-selectedFilters="' .
		json_encode($selectedFilters) .
		'">' .
		$content .
		"</div>";
}
...
function dbe_register_content_filter_entry_block()
{
	if (function_exists("register_block_type")) {
		register_block_type("dbe/content-filter-entry-block", [
			"attributes" => [
...
function dbe_render_content_filter_block($attributes, $content)
{
	...
	foreach ((array) $newFilterArray as $key1 => $filterGroup) {
        ...
        data-canUseMultiple="' .
			json_encode($filterGroup["canUseMultiple"]) .
			'">
        <div class="content-filter-category-name">' .
			$filterGroup["category"] .
			"</div>";

		foreach ($filterGroup["filters"] as $key2 => $tag) {
			$filterList .=
				'<div data-tagIsSelected="false" data-categoryNumber="' .
				$key1 .
				'"
            data-filterNumber="' .
				$key2 .
	...
	return '<div class="wp-block-content-filter' .
		(isset($className) ? " " . esc_attr($className) : "") .
		'"' .
		($blockID === "" ?: ' id="content-filter-' . $blockID . '"') .
		' data-currentSelection="' .
		json_encode($currentSelection) .
		'" data-initiallyShowAll="' .
		json_encode($initiallyShowAll) .
		'" data-matchingOption="' .
		$matchingOption .
		'">' .
		$filterList .
...
function dbe_register_content_filter_block()
{
	if (function_exists("register_block_type")) {
		...
		register_block_type("dbe/content-filter-block", [
			"attributes" =>
				$defaultValues["dbe/content-filter-block"]["attributes"],
...
function dbe_content_filter_add_frontend_assets()
{
	...
	foreach ($presentBlocks as $block) {
		if (
			$block["blockName"] === "dbe/content-filter" ||
			$block["blockName"] === "dbe/content-filter-block"
		) {
			wp_enqueue_script(
