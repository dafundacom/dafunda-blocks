...
function dbe_register_meta() {
	register_meta( 'post', 'dbe_ctt_via', array(
		'show_in_rest' => true,
		'single' => true
	) );
}
...
function dbe_render_click_to_tweet_block( $attributes ) {
    ...
    */
	...
	$url       = apply_filters( 'dbe_click_to_tweet_url', "http://twitter.com/intent/tweet?&text={$tweet_url}&url={$permalink}{$via}" );
...
function dbe_register_click_to_tweet_block() {
	if ( function_exists( 'register_block_type' ) ) {
		...
		register_block_type( 'dbe/click-to-tweet', array(
