...
registerBlockType("dbe/styled-box", {
	title: __("Styled Box"),
	icon: icon,
	...
	edit: compose([
						...
						onClick={() => {
							if (mode === "number") {
								if (i + 1 > block.innerBlocks.length) {
									insertBlocks(
										[
											createBlock("dbe/styled-box-numbered-box-column"),
											createBlock("dbe/styled-box-numbered-box-column"),
		...
		if (mode === "notification") {
			if (
				...
				block.innerBlocks[0].name !== "dbe/styled-box-notification-content"
			) {
				replaceInnerBlocks(block.innerBlocks[0].clientId, [
					createBlock("dbe/styled-box-notification-content"),
				]);
			}
			renderedBlock = (
				<InnerBlocks
					templateLock={"all"}
					template={[["dbe/styled-box-notification-content"]]}
				/>
			);
			...
			if (block.innerBlocks.length > 0) {
				if (
					block.innerBlocks[0].name !== "dbe/styled-box-numbered-box-column"
				) {
				...
				if (title.some((t) => t !== "")) {
					const convertedBlocks = number.map((n, i) =>
						createBlock(
							"dbe/styled-box-numbered-box-column",
							{
								number: n,
								title: title[i],
							},
			...
			if (
				block.innerBlocks.length > 0 &&
				block.innerBlocks[0].name !== "dbe/styled-box-bordered-content"
			) {
				replaceInnerBlocks(block.innerBlocks[0].clientId, [
					createBlock("dbe/styled-box-bordered-content"),
				]);
			}
			renderedBlock = (
				<InnerBlocks
					templateLock={"all"}
					template={[["dbe/styled-box-bordered-content"]]}
				/>
			);
...
registerBlockType("dbe/styled-box-bordered-content", {
	title: __("Bordered Box Content"),
	parent: ["dbe/styled-box"],
	icon: icon,
	category: "dafundablocks",
	supports: {
		inserter: false,
...
registerBlockType("dbe/styled-box-notification-content", {
...
registerBlockType("dbe/styled-box-numbered-content", {
...
registerBlockType("dbe/styled-box-numbered-box-column", {
