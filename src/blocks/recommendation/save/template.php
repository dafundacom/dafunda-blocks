                ...
                <div class="relative aspect-[16/10] md:aspect-[16/7] object-cover object-center w-full overflow-hidden rounded-t-lg">
                    <figure>
                        ...
                        <?php if (isset($list['price']) && $list['price']) : ?>
                            <span class="absolute right-[20px] top-[20px] text-white hover:bg-red-800/90 w-auto h-auto rounded-full leading-none transition duration-200 ease-in-out text-sm md:text-lg font-bold px-6 md:px-11 py-2 md:py-3" style="background:rgba(225, 13, 13, 0.86);"><?= isset($list['pricetag']) ? $list['pricetag'] . " " : "" ?><?= $list['price'] ?></span>
                        <?php endif ?>
                ...
                <div class="p-5">
                    <div class="grid grid-cols-2 gap-3">
                        <?php foreach ($list["olshops"] as $index => $olshop) : ?>
                            <a href="<?= $olshop["url"] ?>" class="py-2  bg-[#EEEEEE] hover:bg-[#c6c3c3] dark:bg-neutral-800 dark:hover:bg-neutral-700 flex justify-center items-center rounded-lg font-bold text-sm duration-200" <?= isset($openNewTab) && $openNewTab ? "target='_blank'" : "" ?>>
