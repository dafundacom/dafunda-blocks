...
function dbe_render_star_rating_block($attributes)
{
	...
	return '<div class="star-rating' .
		...
		($blockID === ""
			...
			: "") .
		'>
                    <div class="star-inner-container">' .
		$stars .
		'</div>
                </div>' .
		($reviewText === ""
			...
			: '<div class="review-text"' .
				($blockID === ""
					? ' style="text-align:' . $reviewTextAlign . ';"'
					: "") .
				">" .
				$reviewText .
				"</div>") .
...
function dbe_register_star_rating_block()
{
	if (function_exists("register_block_type")) {
		...
		register_block_type("dbe/star-rating-block", [
			"attributes" =>
				$defaultValues["dbe/star-rating-block"]["attributes"],
