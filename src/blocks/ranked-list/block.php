			...
			<?php foreach ($lists as $index => $list) : ?>
				<li class="ranked-list-card flex flex-wrap flex-col relative shadow-lg rounded-lg mb-6" data-index="<?= $index ?>">
					...
					<div class="p-5 flex ranked-list-vote w-full justify-center " data-voted="<?= $userVoted ?>">
						<button type="button" class="ranked-list-vote-button flex flex-wrap items-start w-fit opacity-50 text-gray-700 hover:shadow-none <?= in_array(get_current_user_id(), $list["likes"]) ? "opacity-80" : "" ?> <?= in_array(get_current_user_id(), $list["dislikes"]) ? "opacity-30" : "" ?>" data-action="like">
						...
						<button type="button" class="ranked-list-vote-button flex flex-wrap items-start w-fit opacity-50 text-gray-700 hover:shadow-none <?= in_array(get_current_user_id(), $list["dislikes"]) ? "opacity-80" : "" ?> <?= in_array(get_current_user_id(), $list["likes"]) ? "opacity-30" : "" ?>" data-action="dislike">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 inline-block translate-y-[2px]" viewBox="0 0 20 20" fill="currentColor">
