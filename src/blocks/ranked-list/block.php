			...
			<?php foreach ($lists as $index => $list) : ?>
				<li class="ranked-list-card flex flex-wrap flex-col relative shadow-lg rounded-lg mb-6" data-index="<?= $index ?>">
					...
					<div class="p-5 flex ranked-list-vote w-full justify-center" data-voted="<?= $userVoted ?>">
						<button type="button" class="ranked-list-vote-button flex flex-wrap items-start w-fit opacity-50 <?= in_array(get_current_user_id(), $list["likes"]) ? "opacity-80" : "" ?> <?= in_array(get_current_user_id(), $list["dislikes"]) ? "opacity-30" : "" ?>" data-action="like">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 inline-block translate-y-[-2px]" viewBox="0 0 20 20" fill="currentColor">
								<path d="M2 10.5a1.5 1.5 0 113 0v6a1.5 1.5 0 01-3 0v-6zM6 10.333v5.43a2 2 0 001.106 1.79l.05.025A4 4 0 008.943 18h5.416a2 2 0 001.962-1.608l1.2-6A2 2 0 0015.56 8H12V4a2 2 0 00-2-2 1 1 0 00-1 1v.667a4 4 0 01-.8 2.4L6.8 7.933a4 4 0 00-.8 2.4z" />
							</svg>
							<p class="m-0 ml-1 inline-block font-bold text-xl leading-6">
								<?= count($list["likes"]) ?? 0 ?>
							</p>
						</button>
						<button type="button" class="ranked-list-vote-button flex flex-wrap items-start w-fit opacity-50 ml-4 <?= in_array(get_current_user_id(), $list["dislikes"]) ? "opacity-80" : "" ?> <?= in_array(get_current_user_id(), $list["likes"]) ? "opacity-30" : "" ?>" data-action="dislike">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 inline-block translate-y-[2px]" viewBox="0 0 20 20" fill="currentColor">
				...
				el.addEventListener("click", (e) => {
					...
					if (parent.getAttribute("data-voted") != action) {
						...
						el.closest(".ranked-list-vote").querySelectorAll("button").forEach(ell => {
							newClass = "ranked-list-vote-button flex flex-wrap items-start w-fit"
